...
namespace NLayer.Services.Services
{
    public class Services<T> : IServices<T> where T : class
    {
        ...
        public Services(IGenericRepository<T> genericRepository, IUnitOfWork unitOfWork)
        {
        ...
        public async Task<T> GetByIdAsync(int id)
        {
            ...
            if (hasProduct == null) throw new NotFoundException($"{typeof(T).Name}({id}) not found");
            return hasProduct;
        }
